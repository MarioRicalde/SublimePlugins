...
class IndentGuidesListener(sublime_plugin.EventListener):
  ...
  def update_active_guides(self, view):
    ...
    cursorAbsolutes = {}
    for sel in view.sel():
      cursors[view.rowcol(sel.b)[1]] = True
      cursorAbsolutes[sel.b] = True

    active_guides = []
    for guide in self.guides:
      if (not guide.a in cursorAbsolutes) and (view.rowcol(guide.a)[1] in cursors):
        active_guides.append(guide)

    color_scope_name = view.settings().get('indent_guides_active_color_scope_name', DEFAULT_COLOR_SCOPE_NAME)
    view.add_regions("IndentGuidesActiveListener", active_guides, color_scope_name, sublime.DRAW_EMPTY)
